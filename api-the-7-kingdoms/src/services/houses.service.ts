...
export const housesService = {
  async getAll(page: number = 1, pageSize: number = 50, name?: string, region?: string): Promise<House[]> {
    const response = await fetch(EXT_ROUTES.ALL_HOUSES(page, pageSize, name, region));
